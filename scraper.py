...
def main():
    ...
    if args.auto_rdw or (choice and choice == 'Y'):
        ...
        print(f"{'Plate':<12} {'First Reg':<10} {'Mileage':<10} {'Color':<15} {'Dealer':<{max_dealer_len}} {'Price':<12} {'Discount':<10}")
        print("-" * 80)
        for v in sorted_vehicles:
            plate = v.get('license_plate', '')[:11]
            first_reg = v.get('rdw_first_registration', '')[:10]
            mileage = v.get('mileage', '')
            if mileage:
            ...
            if discount:
                discount = f"{discount}%"
            print(f"{plate:<12} {first_reg:<10} {mileage:<10} {color:<15} {dealer:<{max_dealer_len}} {price:<12} {discount:<10}")
        print("=" * 80)
